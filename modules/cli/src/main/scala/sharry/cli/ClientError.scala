...
case class ClientError(reasons: NonEmptyList[String]) extends RuntimeException(reasons.toList.mkString(", ")) {
  ...
  def ++(next: ClientError): ClientError =
    ClientError(reasons concatNel next.reasons)
}
...
object ClientError {
  ...
  def fromResponse(resp: HttpResponse[IO]): IO[Option[ClientError]] =
    if (resp.header.status.isSuccess) IO.pure(None)
  ...
  def onSuccess: Pipe[IO, HttpResponse[IO], HttpResponse[IO]] =
    s => for {
      resp <- s
      err <- Stream.eval(fromResponse(resp))
      r <- err.map(Stream.raiseError(_)).getOrElse(Stream(resp))
    } yield r
